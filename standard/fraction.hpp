...
class fraction{
  ...
  public:
    ...
    fraction(const std::string &buf) : num(0), den(0) {
        int i = (buf[0] == '-');
        for(; i < buf.size() && buf[i] != '/'; i++) num = num * 10 + buf[i] - '0';
        if(i == buf.size()) den = 1;
        else for(i = i + 1; i < buf.size(); i++) den = den * 10 + buf[i] - '0';
        if(buf[0] == '-') num *= -1;
        simplify();
    }
    ...
    static const fraction INF;
};
const fraction fraction::M_INF("-1/0");
const fraction fraction::INF("1/0");
