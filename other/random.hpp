...
struct random_utility{
    ...
    unweighted_graph generate_unweighted_tree(int N){
        unweighted_graph ret(N, false);
        for(int i=1; i<N; i++) ret.add_edge(i, random_int(0, i-1));
        return ret;
    }

    unweighted_graph generate_unweighted_graph(int N, int M, bool is_connected = true){
        assert(M <= N * (N - 1) / 2);
        if(is_connected) M -= N - 1;
        assert(M >= 0);
        unweighted_graph ret(N, false);
        if(is_connected) ret = generate_unweighted_tree(N);
        std::vector<std::vector<int>> from_to(N, std::vector<int>(N, true));
        std::vector<std::pair<int, int>> edges;

        for(unweighted_edge &e : ret.E) from_to[e.from][e.to] = false;
