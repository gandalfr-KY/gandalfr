...
void bfs(const graph<int, is_directed> &G, std::vector<int> &dist,
         std::queue<int> &q) {
    while (!q.empty()) {
        int cu = q.front();
        q.pop();
 ...
 */
template <bool is_directed>
std::vector<int> shortest_path(const graph<int, is_directed> &G,
                               int start_point) {
    std::queue<int> q;
...
template <typename WEIGHT, bool is_directed>
std::vector<WEIGHT> shortest_path(const graph<WEIGHT, is_directed> &G,
                                  int start_point) {
