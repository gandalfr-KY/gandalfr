...
data:
  ...
  bundledCode: "#line 2 \"other/random.hpp\"\n#include <random>\n\n#line 2 \"graph/graph.hpp\"\
    ...
    \ int par, std::vector<int> &ord, std::vector<int> &low,\n        std::vector<edge<WEIGHT>>\
    \ &brds, std::vector<int> &apts) {\n        visited[idx] = true;\n        ord[idx]\
    \ = k++;\n        low[idx] = ord[idx];\n        bool is_apt = false;\n       \
    \ int cnt = 0;\n        for(auto &e : G[idx]) {\n            if(!visited[e.to])\
    ...
    \ {brds, apts};\n    }\n\n    void print() const {\n        std::cout << this->N\
    \ << \" \" << this->E.size() << std::endl;\n        for (const edge<WEIGHT> &e\
    \ : this->E)\n            std::cout << e << std::endl;\n    }\n\n\n};\n#line 5\
    \ \"other/random.hpp\"\n\nstruct random_utility {\n    random_utility(int seed\
    \ = 0) { mt.seed(seed); }\n\n    long long random_64i(long long min_value, long\
    ...
    \ size * 4) {\n            const size_t make_size = static_cast<size_t>(size *\
    \ 1.2);\n\n            while (tmp.size() < size) {\n                while (tmp.size()\
