...
public:
    lowlink(graph<WEIGHT, false> &G) : ord(G.count_nodes(), -1), low(G.count_nodes(), -1) {
        ...
        int C = G.count_connected_components(), N = G.count_nodes(), M = G.count_edges();
        std::vector<bool> is_apt(N, false), is_bridge(M, false);
        auto [decomposed, gr_id, nd_id] = G.decompose();
        std::vector<std::vector<int>> id(N); // i 番目のグラフのノード j の、もとのグラフのノードの番号
        for(int c = 0; c < C; c++) id[c].resize(decomposed[c].count_nodes());
        for(int i = 0; i < N; i++) id[gr_id[i]][nd_id[i]] = i;
